...
public class AnalysisController {
    ...
    public String analyze(@RequestParam("lesson") MultipartFile lesson,
        ...
        try {
            ...
            // Show a friendly message instead of the raw crash page.
            String msg = e.getMessage() != null ? e.getMessage() : "Unknown error";
            if (msg.contains("429") || msg.toLowerCase().contains("rate")) {
                model.addAttribute("error",
                        "The AI service is busy right now (free-tier rate limit). "
                        + "Please wait a minute and try again, or upload a smaller chat file.");
            } else {
                model.addAttribute("error",
                        "Analysis could not be completed. Please try again. (" + msg + ")");
            }
            return "error_page";
        }
    }
